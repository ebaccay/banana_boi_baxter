...
def threshold_segment_naive(gray_img, lower_thresh, upper_thresh):
    ...
    img[gray_img <= lower_thresh] = 0
    img[gray_img >= lower_thresh] = 1
    img[gray_img >= upper_thresh] = 0
...
def edge_detect_naive(gray_img):
    ...
    # TODO: Blur gray_s using Gaussian blurring, convolve the blurred image with
...
def segment_image(img): 
    ...
    # binary = threshold_segment_naive(to_grayscale(img), 0, 255).astype(np.uint8)
